...
router.post("/register", async (req, res, next) => {
  passport.authenticate("register", { session: false }, async (err, user, info) => {
    ...
    const mailOptions = {
      ...
      to: user.email,
      subject: "Bienvenido a animestore",
      html: `
        <div>
          <p>Hola ${user.first_name},</p>
          <p>Gracias por registrarte en animestore. Estamos encantados de tenerte con nosotros.</p>
          <p>Si tienes alguna pregunta, no dudes en ponerte en contacto con nosotros.</p>
          <p>Saludos,<br>El equipo de animestore</p>
        </div>
      `,
    };
    try {
